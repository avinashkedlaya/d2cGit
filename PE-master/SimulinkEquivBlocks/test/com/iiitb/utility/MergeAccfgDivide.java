...
public class MergeAccfgDivide {
	...
	public static Accfg mergeTest(ArrayList<Block> blockList) {
		...
		merged = new Accfg();
		 ...
		 Iterator<String> iterKey =  FetchInputFromLineDivide.adjacencyList.keySet().iterator();
		 ...
		 while(iterKey.hasNext())
		 {
			 ...
			 Iterator<DestNode> tempIter = FetchInputFromLineDivide.adjacencyList.get(key).iterator();
		   ...
		   * As per our model it should contain - Constant , Constant1,Divide
		   * 
		  ...
		  Block[] test = {new Constant("Constant"),new Constant("Constant1"),new Divide("Divide")};
		...
		while(test7Iter.hasNext())
		{
			...
			assertTrue(tempFp, tempFp.equalsIgnoreCase("Divide=(Constant / Constant1)") || tempFp.equalsIgnoreCase("Constant1=2") || tempFp.equalsIgnoreCase("Constant=4"));
	...
	public static Accfg findInputOutputVariableTest(Accfg accfg,
		 ...
		 *  Divide 	  ==> input - Constant & Constant1 , output - Divide 
		 *  
		...
		assertEquals("Divide", ((Variable)output.get(0)).getName());
