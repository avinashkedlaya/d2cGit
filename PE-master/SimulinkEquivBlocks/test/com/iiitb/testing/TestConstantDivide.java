...
public class TestConstantDivide {
	...
	public void test() {
		 ...
		 * "Constant,Constant1,Divide"
		 * in any order
		 * 
		 *  When we iterate lineChildNodesOfSystemNodeList and display node names , 
		 *  there should be  "Line,Line" corresponding to "Constant - Divide" and "Constant1-Divide"
		...
		for (int nodeIter = 0; nodeIter < blockChildNodesOfSystemNodeList
			...
			*/
			...
			else if(blockName.contains("Product"))
			{
				assertEquals("Product_Divide", blockName);
			}
			...
			if (blockName != "" && blockType != null) {
				...
				if (blockName.startsWith(Constants.SUB_SYS)) {
					 ...
					 *    
					 * Based on our model there are only 2 block types : Constant,Divide 
					 * 
					 *  Constant: Value is 4
					 *  
					 *  Constant1: Value is 2
					 *  
					 *  Divide: Input is *\/ and the corresponding key is 1 as per signList
					 *  map in Divide class (Divide Block implementation)
		 ...
		 * 
		 * FP for Divide block - Divide = Constant / Constant1
		...
		for (int nodeIter = 0; nodeIter < lineChildNodesOfSystemNodeList
			 ...
			 FetchInputFromLineDivide testInput = new FetchInputFromLineDivide();
			...
			while(test7Iter.hasNext())
			{
				...
				else if(tempFp.equalsIgnoreCase("Constant1"))
				{
					...
					testFp = "Divide=(Constant / Constant1)";
				
				assertTrue(testFp, testFp.equalsIgnoreCase("Divide=(Constant / Constant1)") || testFp.equalsIgnoreCase("Constant=4") || testFp.equalsIgnoreCase("Constant1=2"));
		 ...
		 * */
			...
			Accfg retAccfg = MergeAccfgDivide.mergeTest((ArrayList<Block>) blockList);
